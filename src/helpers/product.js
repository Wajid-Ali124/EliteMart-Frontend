...
export const cartItemStock = (item, color, size) => {
  if (item.stock) {
    ...
    const selectedVariation = item.variation.find((single) => single.color === color);
    if (selectedVariation && selectedVariation.size && selectedVariation.size.length > 0) {
      const selectedSize = selectedVariation.size.find((single) => single.name === size);
      if (selectedSize) {
        return selectedSize.stock;
      }
    }
  }
  return 0; // or any default value when size information is not available
};
...
// get individual element 2
// const getIndividualItemArray2 = (array) => {
